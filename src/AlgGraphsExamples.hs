...
-- We can get two undriected graphs, one thats a cycle of size (2^k)+1, and one thats made up 
...
-- and not 2-colourable showing that they're equal with eqQRankKEPfrag k constitues a proof that 
...
-- equal to the distance between the corrosponding pair of the spoilers pebbles, or, it is greater 
-- than 2^k-r. If the two spoilers pebbels are on the same graph then it needs to be equal, otherwise 
-- greater than 2^k
...
-- make sure it respects adjancecy since it will be of dist 1 away
